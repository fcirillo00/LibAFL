...
/// Report error and exit forkserver
pub fn report_error_and_exit(status: i32) -> Result<(), Error> {
    ...
    SHM: ShMem,
{
    ...
    pub fn wait_child_termination(&mut self) -> Result<ExitKind, Error> {
        let pid:i32 = self.forkserver.child_pid().as_raw();
    ...
    SP: ShMemProvider<ShMem = SHM>,
{
    ...
    /// If `log_directory` is set, the child will print to file inside the directory
    pub fn log_directory(mut self, path: PathBuf) -> Self {
        self.log_directory = Some(path);
        self
    }
